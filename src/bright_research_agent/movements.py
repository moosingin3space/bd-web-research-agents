...
Cite the rubric row you applied in `interestingness_rationale`
...
- Cross-validate the page's actual publication date against the bucket
  ...
  drop the movement.
- If there are no notable movements, return an empty list.
...
def build_aggregator_agent() -> Agent:
    instructions = """\
You aggregate per-org Movement lists from multiple sub-agents into one final
MovementReport.
...
- `movements`: union of all sub-agent outputs.
...
1. Merge cross-org duplicates (e.g. an acquisition surfaced by both buyer and
   ...
   the counterparty in `summary`. Merge citations.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Discover tech-movement news across a fixed watchlist of AI orgs."
    )
    parser.add_argument(
        "--watchlist",
        type=Path,
